...
/*package*//*
 TypedValue mTmpValue = new TypedValue();
    */
/*package*//*
 final LongSparseArray<WeakReference<ConstantState> > mDrawableCache
            = new LongSparseArray<WeakReference<ConstantState> >(0);
    */
/*package*//*
 final LongSparseArray<WeakReference<ColorStateList> > mColorStateListCache
    ...
    */
/*package*//*
 final LongSparseArray<WeakReference<ConstantState> > mColorDrawableCache
            = new LongSparseArray<WeakReference<ConstantState> >(0);
    */
/*package*//*
 boolean mPreloading;
...
/**
     ...
     * {@link Build.VERSION_CODES#JELLY_BEAN}, this function
...
/**
    ...
    private void clearDrawableCacheLocked(
        ...
        for (int i=0; i<N; i++) {
            WeakReference<ConstantState> ref = cache.valueAt(i);
            if (ref != null) {
                ConstantState cs = ref.get();
                if (cs != null) {
 ...
 Drawable loadDrawable(TypedValue value, int id)
        ...
        if (TRACE_FOR_PRELOAD) {
            ...
            if ((id >>> 24) == 0x1) {
                ...
                if (name != null) Log.d("PreloadDrawable", name);
            }
        }
        ...
        if (cs != null) {
            ...
            if (dr == null) {
                ...
                if (TRACE_FOR_MISS_PRELOAD) {
                    ...
                    if ((id >>> 24) == 0x1) {
                        final String name = getResourceName(id);
                        if (name != null) Log.d(TAG, "Loading framework drawable #"
            ...
            if (cs != null) {
                if (mPreloading) {
                    ...
                    if (isColorDrawable) {
                        ...
                        if (verifyPreloadConfig(changingConfigs,
                            ...
                            if ((changingConfigs&LAYOUT_DIR_CONFIG) == 0) {
                                ...
                                final LongSparseArray<ConstantState> preloads
                    ...
                    synchronized (mAccessLock) {
                        ...
                        //        + " in " + this + ": " + cs);
                        if (isColorDrawable) {
                            mColorDrawableCache.put(key, new WeakReference<ConstantState>(cs));
                        } else {
                            mDrawableCache.put(key, new WeakReference<ConstantState>(cs));
                        }
                    }
                }
            }
    ...
    private Drawable getCachedDrawable(
            ...
            long key) {
        synchronized (mAccessLock) {
            WeakReference<ConstantState> wr = drawableCache.get(key);
...
/*package*//*
 ColorStateList loadColorStateList(TypedValue value, int id)
        ...
        if (TRACE_FOR_PRELOAD) {
            ...
            if ((id >>> 24) == 0x1) {
                final String name = getResourceName(id);
                if (name != null) Log.d("PreloadColorStateList", name);
            }
        }
